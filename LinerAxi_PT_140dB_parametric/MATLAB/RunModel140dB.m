%% Run FEM Model
% The parametric model at 130 dB choosing the start stop and frequency step
...
for i=4:1:length(freq)
    waitbar(i/length(freq),hi)
    freq(i)
    %Calls the numerical model using COMSOL
    LinerAxi_PT_130dB_parametric_v5_2(SPL,d_h,POA,h_h,h_r,cycles,freq(i)) %Run the model and extract results
...
h=figure(1)
hold on;
plot(Exp.PT_IMP(:,1),Exp.PT_IMP(:,2),'-or') %Column 2 Resistance 130dB
plot(Num(:,1),real(Num(:,2)),'-ob')
hold on;
plot(Exp.PT_IMP(:,1),Exp.PT_IMP(:,3),'-xr') %Column 3 Reactance 130 dB
plot(Num(:,1),imag(Num(:,2)),'-xb')
legend('\theta - Exper. Sample Holder','\theta - Numerical','\chi - Exper. Sample Holder','\chi - Numerical','Location','Best')
   string = ['..\Data\Validation130dB']
...
% 
% figure(1)    
% plot(Num(:,1),real(Num(:,6)+0.5),'-ok')
% hold on;
% plot(Num(:,1),imag(Num(:,6)),'-xk')
